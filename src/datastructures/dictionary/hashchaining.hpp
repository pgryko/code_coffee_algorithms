...
class HashChaining {
 ...
 public:
  ...
  HashChaining() noexcept {
    p_array =
        std::make_unique<std::unique_ptr<std::list<std::pair<Key, Value>>>[]>(
            7);
    capacity = 7;
  };
  ...
  std::size_t Hash(Key K) { return std::hash<Key>{}(K) % Capacity(); };
  ...
  std::pair<Key, Value> Get(Key k) {
    ...
    if (p_array[index]) {
      auto list_bucket_iter =
          std::find_if(p_array[index]->begin(), p_array[index]->end(),
...
template <typename Key, typename Value>
void HashChaining<Key, Value>::Push(Key k, Value v) {
  ...
  // Check if key exists
  auto list_bucket_iter = std::find_if(
      p_array[index]->begin(), p_array[index]->end(),
...
void HashChaining<Key, Value>::Resize() {
  ...
  for (auto i(0); i < old_capacity; i++) {
    if (p_new_array[i]) {
      // p_new_array[i] is a unique_ptr, so de-reference it so we can access the
...
// Test class to disable resize - so that we can explicitly force chaining to
// occur and test it
template <typename Key, typename Value>
